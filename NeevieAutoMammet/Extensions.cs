...
internal static class Extensions
{
	...
	{
		return s.Trim().Replace(Constants.Words.SPACED_WRAP_MARKER + "\n", " ")
    ...
    internal static Word[] Words(this string s)
	{
		...
		while (start + len <= s.Length)
		{
			...
			if (start + len == s.Length || " \r\n".Contains(s[start + len]))
			{
				...
				while (start + wordoffset < s.Length &&
				       Constants.Words.PUNCTUATION_CLEANING_LIST.Contains(s[start + wordoffset]) && wordoffset <= len)
				{
					++wordoffset;
				}
				...
				while (start + len - wordlenoffset - 1 > -1 &&
				       Constants.Words.PUNCTUATION_CLEANING_LIST.Contains(s[start + len - wordlenoffset - 1]) &&
				       wordoffset <= len)
				{
